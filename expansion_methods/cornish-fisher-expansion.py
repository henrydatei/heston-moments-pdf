...
def cornish_fisher_expansion(x, mean, variance, skewness, excess_kurtosis):
    ...
    deriv = numerical_derivative(x_values, x_p)
    plt.plot(x_values, x_p)
    plt.plot(x_values, norm.cdf(x_values), 'r--')
    plt.title('x_p')
    plt.show()
    plt.plot(x_values, deriv)
    plt.plot(x_values, norm.pdf(x_values), 'r--')
    plt.title('deriv')
    plt.show()
...
print(cornish_fisher_expansion([0], 0, 1, 0.885, 1.618))
