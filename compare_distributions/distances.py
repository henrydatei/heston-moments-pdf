...
for column in df_qe_logreturn.columns:
    rm_qe.append(rMoments(df_qe_logreturn[column], method=technique, days_aggregate=22, m1zero=True, ret_nc_mom=False).to_numpy())
rm_qe = np.squeeze(np.array(rm_qe))

rm_euler = []
for column in df_euler_logreturn.columns:
    rm_euler.append(rMoments(df_euler_logreturn[column], method=technique, days_aggregate=22, m1zero=True, ret_nc_mom=False).to_numpy())
