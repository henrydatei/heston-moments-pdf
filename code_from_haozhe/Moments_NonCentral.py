...
def MomentsBates(mu, kappa, theta, sigma, rho, lambdaj, muj, vj, t, v0, conditional = True, nc=False):
    ...
    if conditional:
        ...
        if nc:
            #return skewness
            return np.array([mean, M2_v0, M3_v0, M4_v0])              # all are moments
            #return {"m1": mean, "m2": M2_v0, "m3": M3_v0, "m4": M4_v0}
        
        else:
            #return skewness
            return np.array([mean, variance, cumulant3, cumulant4])         # all are cumulants
        ...
        if nc:
            #return skewness
            return np.array([mean, M2_unc, M3_unc, M4_unc])    # all are moments
