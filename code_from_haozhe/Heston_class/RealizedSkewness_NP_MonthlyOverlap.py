...
# Function to get the name of the realized cumulants
def getname_RM(x):
    if x == RM_ACJV:
        return "RM_ACJV"
    elif x == RM_CL:
...
# Function to compute skewness and kurtosis from cumulants
...
def endpoints(df, freq):     # this is equivalent to find the end point of the business calendar month if the df data has been aligned already
    return df.groupby(df.index.to_period(freq)).apply(lambda x: x.index[-1])                # df.resample(freq).apply(lambda x: x.index[-1])

def startpoints(df, freq):   # this is equivalent to find the start point of the business calendar month if the df data has been aligned already
...
def rMomNP(hf, nM):
    ...
    result = pd.DataFrame(columns=['k1', 'k2', 'k3', 'k4'])
...
def RealizedEstimatorsORS(x, nM, NP=True):
    ...
    #m1d = rt.sum() / (nM - 1)
    m1d = foravg_rt.sum() / nM                 # for the average log return of the whole series
    mean = rt.resample('ME').sum().values[-1]   # before "M" is deprecated
...
def rCumulants_MonthlyOverlap(hfData, method, months_overlap=6):
    if method == RM_ACJV:
        cumulants = rMomACJV(hfData, months_overlap)
    elif method == RM_CL:
        cumulants = rMomCL(hfData, months_overlap)
    elif method == RM_NP:
        cumulants = rMomNP(hfData, months_overlap)
    elif method == RM_NP_return:
        cumulants = rMomNP_return(hfData, months_overlap)
    elif method == RM_ORS:
        cumulants = rMomORS(hfData, months_overlap)
    else:
        print("This method for computing cumulants is not implemented")
        cumulants = None
    return cumulants

# Function to compute realized moments
def rMoments_MonthlyOverlap(hfData, method, months_overlap=6, Sk_out=15, Ku_out=20, standardize=False):

    cumulants = rCumulants_MonthlyOverlap(hfData, method, months_overlap)
    
    rSkKu = cumus2skkurt(cumulants)    # output centered var, skewness and kurtosis
    rr = np.array(rSkKu)                             
    ...
    if standardize:
        ...
        outcome = pd.DataFrame(rr, columns=['rMean', 'rVar', 'rSkew', 'rKurt'], index=cumulants.index)
        return outcome
    else:
       
        outcome = np.column_stack((np.array(cumulants), rr[:, 2], rr[:, 3]))
        outcome = pd.DataFrame(outcome, columns=['k1', 'k2', 'k3', 'k4', 'rSkew', 'rKurt'], index=cumulants.index)
        return outcome


def rSkewness_MonthlyOverlap(hfData, method, months_overlap=6):
    cumulants = rCumulants_MonthlyOverlap(hfData, method, months_overlap)
    rSkKu = cumus2skkurt(cumulants)    # output centered var, skewness and kurtosis
    Skewness = rSkKu['rSkew']
    return Skewness
...
def RealizedEstimatorsNP_return(x, nM):        # use log-prices x as inputs (!) now the updated version uses log returns as inputs
